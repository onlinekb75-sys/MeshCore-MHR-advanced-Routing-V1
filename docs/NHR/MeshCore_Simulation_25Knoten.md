# Simulation: MeshCore vs. NHR – 25 Knoten, Rheinland-Topologie

Vergleich des heutigen MeshCore-Routings (netzweiter Flood + „first packet wins") mit dem NHR-Entwurf (metrik-optimaler Pfad + Backbone-Short-Circuit) auf einer realistischen 25-Knoten-Topologie im Korridor **Köln–Bonn–Siebengebirge–Bergisches Land–Eifelrand**.
...
- **NHR:** ETX-optimaler Backbone-Pfad (das Konvergenzziel von Backbone-DV + Best-of-N) + Discovery-Short-Circuit (Client flutet nur bis zum nächsten Repeater, dann Backbone-Unicast).

---

## Ergebnisse (31 cluster-übergreifende Client-Paare)

| Kennzahl | MeshCore (heute) | NHR | Bedeutung |
|---|---|---|---|
...
**Konkretes Beispiel** – MeshCore leitet Köln-Süd→Köln-Nord über Bergisch-Gladbach im Osten um, NHR nimmt den direkten Hochstandort-Hop:
...
- Nicht modelliert: ACK-Retries (würden Umweg-Pfade zusätzlich bestrafen), Tageszeit-/Lastdynamik, Mobilität, Kollisionen auf dem geteilten Kanal (würden netzweite Floods *zusätzlich* benachteiligen → NHR-Vorteil eher größer).
- Das Skript (`sim/nhr_sim.py`) ist parametrisiert und reproduzierbar (Seed 42).
