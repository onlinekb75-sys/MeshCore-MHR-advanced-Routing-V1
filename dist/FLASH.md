# Flashen — MHR-MeshCore auf Heltec V4 (ESP32-S3, Repeater)
...
|---|---|---|
| `MHR-heltec_v4_repeater-factory.bin` | Komplettimage (Bootloader + Partitionen + App) | `0x0` |
| `MHR-heltec_v4_repeater-firmware.bin` | nur App-Image (Update bei bereits vorhandenem Bootloader) | `0x10000` |
...
2. `MHR-heltec_v4_repeater-firmware.bin` wählen, Gerät per USB verbinden, flashen.

## Variante B — esptool (Komplettimage @ 0x0)
```bash
python -m esptool --chip esp32s3 write_flash 0x0 MHR-heltec_v4_repeater-factory.bin
```
Nur App aktualisieren (Bootloader/Partitionen bleiben):
```bash
python -m esptool --chip esp32s3 write_flash 0x10000 MHR-heltec_v4_repeater-firmware.bin
```

## Nach dem Flashen — MHR-Parameter (alle reversibel, ohne Reflash)
...
MHR-Defaults: `rxdelay 10`, `txsnrweight 0.5`. Zum vollständigen Zurücksetzen Upstream-Firmware flashen.
